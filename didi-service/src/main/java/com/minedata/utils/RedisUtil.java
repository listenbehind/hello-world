...
import com.navinfo.mapspotter.foundation.io.Cursor;
...
public class RedisUtil extends DataSource implements Cursor {


    /**
     * redis操作类 1.调用connect连接 2.查询或修改数据 3.结束后调用close tips: 也可以使用try-with-resources Created by
     * gaojian on 2016/1/3.
     */
    private static final Logger logger = Logger.getLogger(RedisUtil.class);
    private ArrayList<Response<String>> popList = new ArrayList<Response<String>>();
    private static final int PIPE_BATCH_COUNT = 10000;

    public Jedis _conn = null; // Redis sqlConnection
    public Pipeline _pipe = null; // For transaction
    private String host;
    private int port;

    public ScanResult<String> _scan = null;
    public int _index = -1;
    ...
    public int open(DataSourceParams params) {
        try {
            ...
            port = params.getPort();
            // 创建连接
            // _conn = new Jedis(host, port, 1000000);
            _conn = JedisPoolSet.getInstance().getResource();
            // 验证密码
    ...
    public void close() {
        try {
            ...
            if (_conn != null) {
                _conn.close();
                // JedisPoolSet.getInstance().returnResource(_conn);
                // JedisPoolSet.getInstance().returnBrokenResource(_conn);
            }
            _conn = null;
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
    ...
    public Object query(String key) {
        try {
            ...
            if (valueType.equals("string")) {
                ...
                value = _conn.lrange(key.getBytes(), 0, -1);
    ...
    public List<byte[]> queryList(byte[] key) {
        try {
    ...
    public void updateList(byte[] key, byte[] value) {
        try {
            if (_pipe != null) {
                _pipe.rpush(key, value);
                pipeCount();
    ...
    public List<String> lpop(String key) {
        try {
            if (_pipe != null) {
                Response<String> lpop = _pipe.lpop(key);
                popList.clear();
                popList.add(lpop);
                while (popList.size() % PIPE_BATCH_COUNT != 0) {
                    lpop = _pipe.lpop(key);
                    popList.add(lpop);
                }
                ArrayList<String> resultList = new ArrayList<String>();
                _pipe.sync();
                for (Response<String> result : popList) {
     ...
     * @return
     */
    public int pipeSet(String key, String value) {
        try {
            if (null == _pipe)
                return -1;
